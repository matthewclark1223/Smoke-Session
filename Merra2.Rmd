...
#loop to calculate polygons means and save vector
  for (i in 1:length (filenames)) {
    ...
    mean<-exact_extract(raster, poly, 'mean') #extracts mean for each park
  ...
  date_file<-paste0("/Volumes/GoogleDrive/My Drive/Local/Projects/Smoke/Smoke-Signal/loop_output/",date)
   newname<-paste0(date_file, ".csv")
write.csv(mean, newname, row.names = FALSE) #save as csv
}

#loads those .csv and makes into single df
list_dates = list.files(path="/Volumes/GoogleDrive/My Drive/Local/Projects/Smoke/Smoke-Signal/loop_output/", pattern="*.csv", full.names=TRUE)
